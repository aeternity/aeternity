...
handle_request('GetTop', _, _Context) ->
    ...
    {ok, Resp} = aec_headers:serialize_to_map(
                   aec_headers:replace_empty_pow(Header)),
...
handle_request('GetBlockByHeight', Req, _Context) ->
    ...
    case aec_chain:get_block_by_height(Height) of
        {ok, Block} ->
            ...
            Resp = aec_blocks:serialize_to_map(
                     aec_blocks:replace_empty_pow(Block)),
            {200, [], Resp};
...
handle_request('GetBlockByHash', Req, _Context) ->
    ...
    {ok, Header} = aec_chain:get_header_by_hash(Hash),
    {ok, HH} = aec_headers:hash_header(Header),
