...
%% get_protocol_parameters reconfigures the running node - it has to, since the
%% point is to prove that a runtime config change reaches the response - and the
%% suite shares one node across every group. Stash the originals here and restore
%% them from end_per_testcase, not only from a try/after inside the case: a CT
%% timetrap kills the case process outright and no `after` runs, which would
%% leave mining > name_claim_bid_timeout at the probe value and turn every claim
%% in the later `naming` group into an auction that never resolves.
init_per_testcase(get_protocol_parameters, Config) ->
    ...
    [{orig_name_claim_bid_timeout, BidTimeout},
...
end_per_testcase(get_protocol_parameters, Config) ->
    %% Guarded on the stashed values being present, so that a failure in
    %% init_per_testcase cannot turn into a second, more confusing failure here.
    %% Each restore is independent: a failing one must not skip the others, nor
    %% end_per_testcase_all/1.
    case ?config(orig_name_claim_bid_timeout, Config) of
        BidTimeout when is_integer(BidTimeout) ->
            catch set_name_claim_bid_timeout(BidTimeout);
        _ ->
            ok
    end,
    case ?config(orig_block_gas_limit, Config) of
        BlockGasLimit when is_integer(BlockGasLimit) ->
            catch rpc(?NODE, application, set_env, [aecore, block_gas_limit, BlockGasLimit]);
        _ ->
            ok
    end,
    end_per_testcase_all(Config);
...
%% breaks this test instead of silently breaking every client. Being a second copy
%% of the handler's own list it cannot detect a type neither knows about; that is
%% what assert_protocol_parameters_tx_type_coverage/0 is for.
...
-define(PROTOCOL_PARAMS_DRY_RUN_GAS_LIMIT, 6000000).
...
get_protocol_parameters(Config) ->
    ...
    #{ <<"min_miner_gas_price">> := MinMinerGasPrice
     ...
     , <<"micro_block_cycle">> := MicroBlockCycle } = NodeSettings,
    %% Aettos amounts are decimal strings; everything else stays a JSON number.
    ...
    ?assertEqual(?PROTOCOL_PARAMS_DRY_RUN_GAS_LIMIT, DryRunGasLimit),
    ?assertEqual(rpc(aec_governance, micro_block_cycle, []), MicroBlockCycle),
    assert_protocol_parameters_block_interval(NodeSettings),
    KnownProtocols = rpc(aec_hard_forks, protocols, []),
    ?assertEqual(lists:sort(maps:to_list(KnownProtocols)),
    ...
    assert_protocol_parameters_block_gas_limit_live(Config, Host, Res),
    ok.

%% The endpoint reports a base fee gas for every transaction type the node knows,
%% except the ones documented as having none. Comparing the lists above to the
%% handler's would be circular; compare them to aetx:tx_types/0 instead, or a
%% type added to aetx and forgotten in aehttp_dispatch_ext is simply missing from
%% every response with nothing failing.
assert_protocol_parameters_tx_type_coverage() ->
...
%% consensus this node runs. See aehttp_dispatch_ext:block_interval_setting/0.
assert_protocol_parameters_block_interval(NodeSettings) ->
    HasMineRate = maps:is_key(<<"expected_block_mine_rate">>, NodeSettings),
    HasChildBlockTime = maps:is_key(<<"child_block_time">>, NodeSettings),
    case rpc(aec_consensus, get_consensus_type, []) of
        pow ->
            ?assert(HasMineRate),
            ?assertNot(HasChildBlockTime),
            ?assertEqual(rpc(aec_governance, expected_block_mine_rate, []),
                         maps:get(<<"expected_block_mine_rate">>, NodeSettings));
        pos ->
            ?assertNot(HasMineRate),
            ?assert(HasChildBlockTime),
            ?assertEqual(rpc(aeu_env, user_config,
                             [[<<"chain">>, <<"consensus">>, <<"0">>, <<"config">>,
                               <<"child_block_time">>]]),
                         {ok, maps:get(<<"child_block_time">>, NodeSettings)})
    end,
    ok.
...
assert_protocol_parameters_int_as_string(Host, Res) ->
    ...
    ?assertMatch(X when is_binary(X),
                 maps:get(<<"mempool_tx_ttl">>, maps:get(<<"node_settings">>, StrRes))),
    ?assertEqual(maps:get(<<"min_miner_gas_price">>, maps:get(<<"node_settings">>, Res)),
                 maps:get(<<"min_miner_gas_price">>, maps:get(<<"node_settings">>, StrRes))),
    ...
    ?assertMatch(X when is_binary(X), maps:get(<<"block_gas_limit">>, StrP)),
...
%% request rather than derived once per protocol. Raising the limit is the safe
%% direction to probe: it cannot invalidate a block the node has already accepted.
assert_protocol_parameters_block_gas_limit_live(Config, Host, Res) ->
    Original = ?config(orig_block_gas_limit, Config),
    Raised = Original + 1000,
    try
        ok = rpc(application, set_env, [aecore, block_gas_limit, Raised]),
        {ok, 200, Bumped} = http_request(Host, get, "protocol-parameters", []),
        BumpedProtocols = maps:get(<<"protocols">>, Bumped),
        ?assertEqual(lists:duplicate(length(BumpedProtocols), Raised),
                     [maps:get(<<"block_gas_limit">>, P) || P <- BumpedProtocols])
    after
...
assert_protocol_consensus_parameters(#{ <<"version">> := Vsn } = P) ->
    ...
    [ begin
          ...
          ?assertEqual(rpc(aec_governance, state_gas_per_block, [Type]), {Part, Whole}),
          %% ?ORACLE_STATE_GAS_PER_YEAR over ?EXPECTED_BLOCKS_IN_A_YEAR_FLOOR
          ?assertEqual({32000, 175200}, {Part, Whole})
      end
      || {Type, Name} <- StateGasTypes ],

    %% Name fees are decimal strings, indexed 1..31 by name length. Pin both ends of
    %% the table: the head is ~2^69, the value that must never come back as a JSON
    %% number, and the tail catches an off-by-one in the index.
    ?assertEqual(31, length(NameClaimFees)),
    ?assert(lists:all(fun is_binary/1, NameClaimFees)),
    ...
    %% Completeness, not just soundness: sweep a range wider than the VM/ABI
    %% versions that exist and require the endpoint to report exactly the legal set.
