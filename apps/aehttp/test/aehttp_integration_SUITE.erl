...
channel_sign_tx(Pubkey, ConnPid, Privkey, Method, Config) ->
    ...
    EncSignedTx =
        aeser_api_encoder:encode(transaction,
                                 aetx_sign:serialize_to_binary(SignedTx)),
    ws_send_tagged(ConnPid, Method, #{tx => EncSignedTx}, Config),
    {Tx, Updates}.
...
channel_create(Config, IConnPid, RConnPid) ->
    ...
    {CrTx, Updates} = channel_sign_tx(IPubkey, IConnPid, IPrivkey, <<"channels.initiator_sign">>, Config),
    {ok, #{<<"event">> := <<"funding_created">>}} = wait_for_channel_event(RConnPid, info, Config),
    %% responder gets to sign a create_tx
    {CrTx, Updates} = channel_sign_tx(RPubkey, RConnPid, RPrivkey, <<"channels.responder_sign">>, Config),
...
channel_update(#{initiator := IConnPid, responder :=RConnPid},
    ...
    %% starter signs the new state
    {UnsignedStateTx, Updates} = channel_sign_tx(StarterPubkey, StarterPid, StarterPrivkey, <<"channels.update">>, Config),
    ...
    {UnsignedStateTx, Updates} = channel_sign_tx(AcknowledgerPubkey, AcknowledgerPid, AcknowledgerPrivkey, <<"channels.update_ack">>, Config),
...
sc_ws_close_mutual_(Config, Closer) when Closer =:= initiator
    ...
    CloseMutual =
        fun(CloserPubkey, CloserConn, CloserPrivkey, OtherPubkey, OtherConn, OtherPrivkey) ->
            ...
            {ShTx, Updates} = channel_sign_tx(OtherPubkey, OtherConn, OtherPrivkey, <<"channels.shutdown_sign_ack">>, Config),
            ShTx
        end,
    ShutdownTx =
        case Closer of
...
sc_ws_deposit_(Config, Origin) when Origin =:= initiator
    ...
    ws_send_tagged(SenderConnPid, <<"channels.deposit">>, #{amount => 2}, Config),
    {UnsignedStateTx, Updates} = channel_sign_tx(SenderPubkey, SenderConnPid, SenderPrivkey, <<"channels.deposit_tx">>, Config),
    {ok, #{<<"event">> := <<"deposit_created">>}} = wait_for_channel_event(AckConnPid, info, Config),
    {UnsignedStateTx, Updates} = channel_sign_tx(AckPubkey, AckConnPid, AckPrivkey, <<"channels.deposit_ack">>, Config),
    ct:log("Unsigned state tx ~p", [UnsignedStateTx]),
...
sc_ws_withdraw_(Config, Origin) when Origin =:= initiator
    ...
    {ok, #{<<"reason">> := <<"not_a_number">>}} =
        wait_for_channel_event(SenderConnPid, error, Config),
    ws_send_tagged(SenderConnPid, <<"channels.withdraw">>, #{amount => 2}, Config),
    {UnsignedStateTx, Updates} = channel_sign_tx(SenderPubkey, SenderConnPid, SenderPrivkey, <<"channels.withdraw_tx">>, Config),
    {ok, #{<<"event">> := <<"withdraw_created">>}} = wait_for_channel_event(AckConnPid, info, Config),
    {UnsignedStateTx, Updates} = channel_sign_tx(AckPubkey, AckConnPid, AckPrivkey, <<"channels.withdraw_ack">>, Config),
    ct:log("Unsigned state tx ~p", [UnsignedStateTx]),
...
update_volley_(FirstPubkey, FirstConnPid, FirstPrivkey, SecondPubkey, SecondConnPid, SecondPrivkey, Config) ->
    {UnsignedStateTx, Updates} = channel_sign_tx(FirstPubkey, FirstConnPid, FirstPrivkey, <<"channels.update">>, Config),

    % acknowledger signs update_ack
    {ok, #{<<"event">> := <<"update">>}} = wait_for_channel_event(SecondConnPid, info, Config),
    {UnsignedStateTx, Updates} = channel_sign_tx(SecondPubkey, SecondConnPid, SecondPrivkey,
