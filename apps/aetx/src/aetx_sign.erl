...
verify_signatures([], _Bin, Sigs, _Protocol, _SignPrefix) ->
    %% No signer to check against: either nothing to check at all - a fully
    %% GA-authenticated transaction - or signatures that nobody accounts for.
    %% Kept ahead of the payload construction below, which neither case uses.
    verify_signatures_([], Sigs, undefined, undefined);
...
verify_signatures_(PubKeys, Sigs, _BinForNetwork, _HashPayload) ->
    %% An invalid pubkey, or signatures left over after every signer.
    lager:debug("Signature check failed: ~p ~p", [PubKeys, Sigs]),
    {error, signature_check_failed}.

%% The payloads depend on the transaction alone, so build them once per
%% transaction. The hashed one stays lazy: only needed once a signature fails
...
%% 'none' before Lima, {pending, ...} until the hashed payload is first needed,
%% and the payload itself from then on.
hash_payload(none) ->
    none;
...
hash_payload(Payload) when is_binary(Payload) ->
    Payload.
            ...
            case hash_payload(HashPayload) of
                ...
                HashForNetwork ->
                    case enacl:sign_verify_detached(Sig, HashForNetwork, PubKey) of
                        true  -> {ok, Acc ++ Left, HashForNetwork};
                        false -> verify_one_pubkey_(Left, PubKey, BinForNetwork,
                                                    HashForNetwork, [Sig | Acc])
                    end
            end
    end;
verify_one_pubkey_([], _PubKey, _BinForNetwork, _HashPayload, _Acc) -> % no more signatures
    error.
