...
-define(FEES, 10).
-define(MINER_REWARD1, 400).
-define(MINER_REWARD2, 401).
-define(POW_REWARD, 20).
...
rewards_calculation_test_() ->
    [{"Both miners are honest, previous key block is not genesis -> no locked",
     ...
     {"Miner1 is fraudulent, miner2 is honest, previous key block is genesis -> miner1 fees are locked, miner2 gets pow reward",
      fun() ->
          {_B1Amt  = 0,
           _B2Amt  = ?B2_FEES + ?MINER_REWARD2 + ?POW_REWARD,
           _Locked = ?B1_FEES + ?MINER_REWARD1 - ?POW_REWARD} =
     ...
     {"All fraudulent miner's fees are locked",
      fun() ->
          ...
          K3Coinbase = 1007,
          {_, 0, Locked1} =
              ?TEST_MODULE:calc_rewards(false, true, K1Fees, K1Coinbase,
                                        K2Coinbase, K1FraudReward, false),
          ?assertEqual(Locked1, K1Fees * 6 div 10),
          {0, _, Locked2} =
              ?TEST_MODULE:calc_rewards(true, false, K2Fees, K2Coinbase,
...
rewards(K1Fraud, K2Fraud, IsK1Genesis) ->
    ?TEST_MODULE:calc_rewards(K1Fraud, K2Fraud, ?FEES, ?MINER_REWARD1,
                              ?MINER_REWARD2, ?POW_REWARD, IsK1Genesis).
