...
generate_block_chain_by_difficulties_with_nonce_and_coinbase(
  GenesisBlock, Difficulties, Nonce) ->
...
generate_block_chain_by_difficulties_with_nonce_and_coinbase(
  ...
  MinerAccount) ->
    ...
    {GD, GD} = {float(GenesisDifficulty), aec_blocks:difficulty(GenesisBlock)},
    lists:reverse(
      lists:foldl(
        fun(D, [PrevB | _] = BC) ->
                {ok, PrevHH} = aec_blocks:hash_internal_representation(PrevB),
                H = 1 + aec_blocks:height(PrevB),
                B = #block{height = H,
                           prev_hash = PrevHH,
                           target = D,
                           nonce = Nonce,
                           txs = [a_signed_coinbase_tx(MinerAccount, H)]},
                [B | BC]
        end,
        [GenesisBlock],
        OtherDifficulties)).

a_signed_coinbase_tx(Account, _AccountNonce) ->
     ...
     [{"The alternative header chain has a different genesis hence its amount of work cannot be compared",
      ...
      {"The alternative header chain does not have more work - case alternative chain is less high",
       fun() ->
               ...
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 2, 2], 111),
               AltBC = [B0, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 3], 222),
      ...
      {"The alternative header chain does not have more work - case alternative chain is higher",
       fun() ->
               %% Generate the two header chains.
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 3], 111),
               AltBC = [B0, _, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1], 222),
      ...
      {"The alternative chain has the same amount of work, hence is to be ignored because received later",
       fun() ->
               %% Generate the two header chains.
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 2], 111),
               AltBC = [B0, _, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1], 222),
               MainHC = [H0, HM1] = header_chain_from_block_chain(MainBC),
               AltHC = [H0, _, _] = header_chain_from_block_chain(AltBC),
      ...
      {"The alternative header chain has more work - case alternative chain is higher. Force chain excluding genesis.",
       fun() ->
               ...
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 2], 111),
               AltBC = [B0, _, _, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1, 1], 222),
               MainHC = [H0, HM1] = header_chain_from_block_chain(MainBC),
      ...
      {"The alternative header chain has more work - case alternative chain is less high",
       fun() ->
               %% Generate the two header chains.
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1], 111),
               AltBC = [B0, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 3], 222),
               MainHC = [H0, _, HM2] = header_chain_from_block_chain(MainBC),
      ...
      {"The alternative chain initially has more work, but concurrent insertion in chain service makes it not have more work any longer. I.e. concurrent insertions in the chain service do not result in sub-optimal choice.",
       fun() ->
               ...
               %% Generate the two header chains.
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _, _, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1, 2], 111),
               AltBC = [B0, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 3], 222),
...
longest_block_chain_test_() ->
    {foreach,
     ...
     [{"The alternative block chain has more work - case both main and alternative chains with all blocks",
       fun() ->
               %% Generate the two block chains.
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _, B2] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1], 111),
               AltBC = [B0, _, _, BA3] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1, 1], 222),
               MainHC = [H0, _, HM2] = header_chain_from_block_chain(MainBC),
               AltHC = [H0, _, _, HA3] = header_chain_from_block_chain(AltBC),
      ...
      {"The alternative block chain has more work - case main chain with only a block (not contiguous to genesis), and alternative chain with only block corresponding to top header (not contiguous to genesis)",
       fun() ->
               %% Generate the two block chains.
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _, B2] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1], 111),
      ...
      {"The alternative block chain has more work - case main chain with two blocks contiguous to genesis, and alternative chain with only block corresponding to header before top header (contiguous to genesis)",
       fun() ->
               %% Generate the two block chains.
               B0 = fake_genesis_block_with_difficulty(),
               MainBC = [B0, _, B2] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1], 111),
               AltBC = [B0, _, BA2, _] = generate_block_chain_by_difficulties_with_nonce_and_coinbase(B0, [1, 1, 1, 1], 222),
               MainHC = [H0, _, HM2] = header_chain_from_block_chain(MainBC),
               AltHC = [H0, _, _, HA3] = header_chain_from_block_chain(AltBC),
