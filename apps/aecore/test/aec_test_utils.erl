...
create_keyblock_with_state([{PrevBlock, TreesIn} | _] = Chain, MinerAccount, BeneficiaryAccount, BlockCfg) ->
    ...
    Version = get_config(version, BlockCfg,
                         fun() ->
                                 {ok, V} = aec_hard_forks:protocol_effective_at_height(Height),
                                 V
                         end),
    PrevVersion = aec_blocks:version(PrevBlock),
    Trees1 = aec_trees:perform_pre_transformations(TreesIn, Height, Version, PrevVersion),
...
extend_block_chain_with_key_blocks1(N, MinerAccount, BeneficiaryAccount, [BlockCfg | BlockCfgs], Acc) when N > 0 ->
    {B, S} = aec_test_utils:create_keyblock_with_state(Acc, MinerAccount, BeneficiaryAccount, BlockCfg),
