...
fault_injection(Config) ->
    ...
    T = 0.1, %% Percent of time we are crashing in rocksdb :)
    Pid = rpc:call(N, erlang, spawn, [
        fun() ->
            meck:new(mnesia_rocksdb_lib, []),
            meck:expect(mnesia_rocksdb_lib, put, fun(Ref, K, V, Opts) -> case random:uniform() < T of false -> {error, hmm}; true -> meck:passthrough([Ref, K, V, Opts]) end end),
            meck:expect(mnesia_rocksdb_lib, write, fun(Ref, L, Opts) -> case random:uniform() < T of false -> {error, hmm}; true -> meck:passthrough([Ref, L, Opts]) end end),
            meck:expect(mnesia_rocksdb_lib, delete, fun(Ref, L, Opts) -> case random:uniform() < T of false -> {error, hmm}; true -> meck:passthrough([Ref, L, Opts]) end end),
            receive
                {done, P} ->
    ...
    State0 = lists:usort(rpc:call(N, mnesia, dirty_all_keys, [aec_account_state])),
    {ok, []} = rpc:call(N, aec_chain_state, insert_block, [MB0S]), %% NOOOOO. This shouldn't make it...
    Pid ! {done, self()},
    receive
        done ->
