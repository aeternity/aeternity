...
abort_large_insertion(Config) ->
    ...
    MutatorFun = fun() ->
			 ...
			 rpc:call(N, aec_conductor, post_block, [MB1S])
                 end,
    %% Race it!
    {_Pid, MRef} = spawn_monitor(fun() ->
					 exit({ok, [MutatorFun() || _ <- lists:seq(1, 100)]})
				 end),
    [CheckFun() || _ <- lists:seq(1, 100)],
    PidResult = receive {'DOWN', MRef, _, _, PR} -> PR end,
    ct:log("PidResult = ~p", [PidResult]),
    {ok, MRes} = PidResult,
    case lists:all(fun(X) ->
			   X == {error, invalid_transactions_in_block}
		   end, MRes) of
	true -> ok;
	false -> error({bad_returns, MRes})
    end,
    ok.
