...
%% Read and delete from the pool the transactions with highest fee -
%% up to the specified number of transactions.  The transactions are
%% returned in order of decreasing fee.
%%
...
%% pool.
-spec pop(MaxNumberOfTxs::pos_integer()) -> {ok, [signed_tx()]}.
pop(MaxNumberOfTxs) ->
    gen_server:call(?SERVER, {pop, MaxNumberOfTxs}).
...
handle_call({pop, MaxNumberOfTxs}, _From, State)
  when is_integer(MaxNumberOfTxs), MaxNumberOfTxs > 0 ->
    Db = State#state.txs,
    {ok, Txs} = pool_db_peek(Db, MaxNumberOfTxs),
    NewDb =
        lists:foldl(
          fun(T, DbIn) ->
                  {ok, DbOut} = pool_db_delete(DbIn, pool_db_key(T)),
                  DbOut
          end,
          Db,
          Txs),
