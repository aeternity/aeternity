...
%%%
%%% Usage:
%%%  - When setting up, add trusted peers calling `update/7`.
%%%  - When receiving a peer through gossip, call `update/7`.
%%%  - To select a peer to connect to, call `select/4`.
%%%  - When an outbound connection failed, call `reject/3`.
%%%  - When an outbound connection is closed, call `release/3`.
%%%  - When an inbound connection is established and the first gossip
%%%    message is received, call `update/7`, `verify/3` and `selected/4`.
%%%  - When building a gossip message, call `subset/3`.
%%%
%%% To support selecting peers from a new address group, a filter function
%%% must be given to `select/4` that will reject peers from the current
%%% connections address groups.
%%%
%%% @end
%%%=============================================================================
...
-define(MAX_LOOKUP_SIZE_INC, 128).

%% Backoff lookup table for standby duration.
...
%% The default number of possible buckets for the same peer and source address groups.
-define(DEFAULT_UNVER_GROUP_SHARD,         4).
%% The default maximum number of unverified peer references in the unverified pool.
...
-record(?MODULE, {
  ...
  lookup_all             :: lookup(),
  % The randomized list of verified peers that are neither selected nor rejected.
  lookup_verif           :: lookup(),
  % The randomized list of unverified peers that are neither selected nor rejected.
  lookup_unver           :: lookup(),
  ...
  % If a strong random number should be used as an offset of weak ransdom number
  % for `select/4` and `subset/3`.
...
-type select_fun() :: fun((state(), millitimestamp(), int_filter_fun() | undefined)
...
%% Returns a binary representing the address group of given IP address.
...
%%
%% Options:
%%  - `verif_bcount`: Number of buckets in the verified pool. Default: `256`.
%%  - `verif_bsize`: Verified pool's buckets size. Default: `32`.
%%  - `verif_group_shard`: The number of possible buckets for the same peer
%%    address group. Default: `8`.
%%  - `unver_bcount`: Number of buckets in the unverified pool. Default: `1024`.
%%  - `unver_bsize`: Unverified pool's bucket size. Default: `64`.
%%  - `unver_source_shard`: The default number of possible buckets for the same
%%    source address group. Default: `64`.
%%    This **MUST** be a multiple of `unver_group_shard`.
%%  - `unver_group_shard`: The default number of possible buckets for the same
%%    peer and source address groups. Default: `4`.
%%  - `unver_max_refs`: Maximum number of peer references in the unverified pool.
%%    Default: `8`.
%%  - `eviction_skew`: The skew of the eviction algorithm. `1.0` means no skew,
%%    and the larger the value the more the eviction is skewed toward the oldest
%%    peers. Default: `1.2`.
%%  - `select_verif_prob`: The probability of selecting a verified peer when
...
%%    Default: `0.5`.
%%  - `max_update_lapse`: The maximum time after which a peer is removed if
%%    never updated in milliseconds. Default: `30 days`.
%%  - `secret`: A secret binary used to further randomize the pool. If not
%%    specified a random one will be generated.
%%  - `seed`: A seed for the weak random generator. If not specified, a seed
%%    will be generated from a strong source of randomness.
%%  - `disable_strong_random`: If `true` no strong random number will be used
%%    as offset for `subset/3` and `select/4`; this ensure reproducibility if
%%    a specific seed was provided.
...
new(Opts) ->
  ...
  RandState = rand:seed_s(?RAND_ALGO, Seed),
  ?ST{
    secret = Secret,
    rand = RandState,
    peers = #{},
...
%%
%% Descriminators:
%%  - `all` + `both`: Returns the total number of pooled peers.
%%  - `all` + `verified`: Returns the total number of verified peers.
%%  - `all` + `unverified`: Returns the total number of unverified peers.
%%  - `available` + `both`: Returns the number of peers selectable peers.
%%  - `available` + `verified`: Returns the number of selectable verified peers.
%%  - `available` + `unverified`: Returns the number of selectable unverified peers.
%%
%% Note that the result of `available` peers do not take into account any
%% additional restrictions that could be given to `select/3` or `select/4`.
...
%% @doc Returns where the given peer identifier is pooled and if it is available.
...
peer_state(St, PeerId) ->
  ...
  case find_peer(St, PeerId) of
    ...
    Peer ->
      IsAvailable = not (peer_is_selected(Peer) or maps:is_key(PeerId, Standby)),
...
%%
%% Takes a time in millisecond as it would be returned from
...
%%
%% Some opaque term can be specified; this term will be returned by `select/4`
%% alongside the peer identifier. This could be used by the caller to store
%% extra connection information like protocol and port number. This extra
%% information will **NOT** be updated if the peer already exists.
...
%% the **FIRST** time it was added by `update/7`.
%%
%% A filtering function can be specified to limit the possible results.
-spec subset(state(), all | pos_integer(), ext_filter_fun() | undefined)
  -> {[ext_peer()], state()}.
subset(St, Size, ExtFilterFun) ->
  ?ST{rand = RState, use_rand_offset = ROffset, lookup_all = Lookup} = St,
  IntFilterFun = wrap_filter_fun(St, ExtFilterFun),
  {PeerIds, RState2} = lookup_sample(Lookup, RState, ROffset, Size, IntFilterFun),
...
%% selected:
%%  - `verified`: Only verified peers will get selected.
%%  - `unverified`: Only unverified peers will get selected.
%%  - `both`: A peer will be selected from any of the pools; the pool the peer
...
%% `release/3` has to be called or the standby time for rejection has to be
%% exhausted after calling `reject/3.
%%
...
%% identifier and its extra data given the **FIRST** time it was added by
%% `update/7`.
%%
...
%% **NEVER** be returned by `select/3` again until it is released with
%% `release/2` or marked as rejected with `reject/2` and the standby time is
%% exausted.
-spec select(state(), millitimestamp(), select_target(), ext_filter_fun() | undefined)
  -> {selected, ext_peer(), state()}
   | {wait, milliseconds(), state()}
   | {unavailable, state()}.
select(St, Now, Target, FilterFun) ->
...
%% Every calls should be given a time greater or equal to the last call.
%%
%% This call is intended to mark a peer retrieved through other mean than
%% `select/4` as selected. For example a peer to was just added to the pool
%% with `update/7` and `verify/3`.
%%
%% If the peer wasn't selected, this call mark it as selected; it means that it
%% will **NEVER** be returned by `select/3` again until it is released with
%% `release/2` or marked as rejected with `reject/2`.
%%
...
-spec selected(state(), millitimestamp(), peer_id()) -> state().
selected(St, Now, PeerId) ->
...
-spec randint(rand_state(), non_neg_integer()) -> {non_neg_integer(), rand_state()}.
randint(RandState, Max) ->
  {RandInt, RandState2} = rand:uniform_s(Max, RandState),
  {RandInt - 1, RandState2}.
...
-spec skewed_randint(rand_state(), non_neg_integer(), number()) -> {non_neg_integer(), rand_state()}.
skewed_randint(RandState, Max, Skew) ->
  {RandFloat, RandState2} = rand:uniform_s(RandState),
  {floor(Max * math:pow(RandFloat, Skew)), RandState2}.

-spec safe_min(undefined | number(), number()) -> number().
safe_min(undefined, Value) -> Value;
safe_min(Value1, Value2) -> min(Value1, Value2).

%% Returns the time in miliseconds a peer should stay in standby after rejection.
...
should_add_ref(RandState, RefCount) ->
  {RandVal, RandState2} = randint(RandState, floor(math:pow(2, RefCount))),
  {RandVal =:= 0, RandState2}.
...
-spec update_peer(state(), millitimestamp(), peer_id(), peer_addr(), peer_addr(), boolean(), extra()) -> ignored | {updated, state()}.
...
select_order(St, both) ->
  ?ST{rand = RandState, select_verif_prob = Prob} = St,
  IntProb = floor(Prob * 1000),
  {RandInt, RandState2} = randint(RandState, 1001),
  St2 = St?ST{rand = RandState2},
...
-spec select_peer(state(), millitimestamp(), select_target(), ext_filter_fun() | undefined)
  -> {unavailable, state()} | {selected, peer_id(), state()} | {wait, milliseconds(), state()}.
...
-spec select_available_peer(state(), millitimestamp(), [select_fun()], int_filter_fun() | undefined)
...
-spec select_standby_peer(state(), millitimestamp(), select_target(), int_filter_fun() | undefined)
...
-spec make_bucket_filtering_fun(state(), millitimestamp(), peer_id() | undefined)
  -> bucket_filter_fun().
  ...
  fun
    ...
    (PeerId) ->
      ...
      case Peer of
        ...
        #peer{update_time = Time} when (Now - Time) > MaxLapse -> remove;
        _ -> evict
      end
  end.
...
verified_bucket_index(St, PeerAddr) ->
  ?ST{secret = Secret, verif_pool = Pool, verif_group_shard = GroupShard} = St,
  ...
  case peer_state(Peer) of
    ...
    unverified ->
      ...
      case verified_add(St, Now, Peer) of
        {verified, St2} ->
          % Succeed to add to the verified pool; remove it from unverified.
          St3 = unverified_del(St2, PeerId),
          {verified, St3};
        {ignored, St2} ->
          % Failed to add to verified pool; keep the peer as unverified.
          {unverified, St2}
      end
  end.
...
%% It **only** deletes the peer from the verified pool and verified lookup table.
...
-spec verified_make_space_for(state(), millitimestamp(), non_neg_integer(), peer_id())
...
verified_make_space_for(St, Now, BucketIdx, PeerId) ->
  ...
  ?ST{rand = RandState, peers = Peers, verif_pool = Pool} = St,
  ...
  case pool_make_space(Pool, RandState, BucketIdx, BucketFilterFun, SortKeyFun) of
    no_space -> {no_space, St};
    {free_space, [], undefined, RandState2, Pool2} ->
      St2 = St?ST{rand = RandState2, verif_pool = Pool2},
      {free_space, St2};
    {free_space, RemovedIds, undefined, RandState2, Pool2} ->
      Pool3 = pool_update_size(Pool2, -length(RemovedIds)),
      St2 = St?ST{rand = RandState2, verif_pool = Pool3},
      ...
      {free_space, St3};
    {free_space, [], EvictedId, RandState2, Pool2} ->
      % Try downgrading the evicted peer, if any; delete it if downgrade fail.
      Pool3 = pool_update_size(Pool2, -1),
      St2 = St?ST{rand = RandState2, verif_pool = Pool3},
...
verified_select(St, _Now, FilterFun) ->
  ...
  {Result, RandState2} = lookup_select(Lookup, RState, ROffset, FilterFun),
  {Result, St?ST{rand = RandState2}}.
...
-spec unverified_bucket_index(state(), peer_addr(), peer_addr()) -> non_neg_integer().
...
%% the unverified pool.
-spec unverified_maybe_add(state(), millitimestamp(), peer_id(), peer_id() | undefined)
  -> {verified, state()} | {unverified, state()} | {ignored, state()}.
unverified_maybe_add(St, Now, PeerId, KeepPeerId) ->
  ...
  case peer_state(Peer) of
    ...
    unverified  ->
      ?ST{rand = RandState, unver_pool = Pool} = St,
      #peer{uidxs = Idxs} = Peer,
      case pool_should_add_ref(Pool, RandState, Idxs) of
        {false, RandState2} ->
          {unverified, St?ST{rand = RandState2}};
        {true, RandState2} ->
          St2 = St?ST{rand = RandState2},
...
-spec unverified_add_reference(state(), millitimestamp(), peer(), peer_id() | undefined) -> state().
unverified_add_reference(St, Now, Peer, KeepPeerId) ->
  #peer{id = PeerId, addr = PeerAddr, source = SourceAddr, uidxs = Idxs} = Peer,
  ?assertNotEqual([], Idxs),
  BucketIdx = unverified_bucket_index(St, SourceAddr, PeerAddr),
...
%% or evicted.
-spec unverified_make_space(state(), millitimestamp(), non_neg_integer(), peer_id() | undefined)
  -> {no_space, state()} | {free_space, state()}.
unverified_make_space(St, Now, BucketIdx, KeepPeerId) ->
  % When evicting we want to skew the random selection in favor of the peers
  % updated the longest time ago, but we never evict selected peers.
  ?ST{rand = RandState, peers = Peers, unver_pool = Pool} = St,
  ...
  case pool_make_space(Pool, RandState, BucketIdx, BucketFilterFun, SortKeyFun) of
    no_space -> {no_space, St};
    {free_space, RemovedIds, EvictedId, RandState2, Pool2} ->
      St2 = St?ST{rand = RandState2, unver_pool = Pool2},
      St3 = lists:foldl(fun
        (undefined, S) -> S;
        (PeerId, S) ->
...
-spec peer_new(peer_id(), peer_addr(), peer_addr(), boolean(), extra()) -> peer().
...
-spec peers_lookup_add(peer_map(), rand_state(), peer_id(), lookup(), pos_integer())
  -> {lookup(), rand_state(), peer_map()}.
peers_lookup_add(Peers, RandState, PeerId, Lookup, RecField) ->
  #{PeerId := Peer} = Peers,
  ?assertEqual(undefined, element(RecField, Peer)),
  case lookup_add(Lookup, RandState, PeerId) of
    {Idx, undefined, RandState2, Lookup2} ->
      Peer2 = setelement(RecField, Peer, Idx),
      Peers2 = Peers#{PeerId := Peer2},
      {Lookup2, RandState2, Peers2};
    {Idx, {MovedPeerIdx, MovedPeerId}, RandState2, Lookup2} ->
...
pool_new(Count, Size, MaxRefs, EvicSkew) ->
  #pool{
    size = 0,
    max_refs = MaxRefs,
    skew  = EvicSkew,
...
-spec pool_should_add_ref(pool(), rand_state(), [non_neg_integer()]) -> {boolean(), rand_state()}.
pool_should_add_ref(Pool, RandState, RefIdxs) ->
  #pool{max_refs = MaxRefCount} = Pool,
  RefCount = length(RefIdxs),
  case RefCount < MaxRefCount of
    true -> should_add_ref(RandState, RefCount);
    false -> {false, RandState}
  end.
...
-spec pool_make_space(pool(), rand_state(), non_neg_integer(), bucket_filter_fun(), bucket_sort_key_fun())
  -> no_space | {free_space, [peer_id()], peer_id() | undefined, rand_state(), pool()}.
pool_make_space(Pool, RandState, BucketIdx, FilterFun, SortKeyFun) ->
  ?assert(BucketIdx < Pool#pool.bucket_count),
  ...
  case BucketSize < MaxBucketSize of
    true ->
      {free_space, [], undefined, RandState, Pool};
    false ->
      ...
      case {Removed, EvictableSize} of
        {[_|_], _} ->
          ...
          {free_space, Removed, undefined, RandState, Pool2};
        {[], 0} ->
        ...
        _ ->
          ...
          {EvictedIdx, RandState2} = skewed_randint(RandState, EvictableSize, Skew),
          {_, EvictedValue} = lists:nth(EvictedIdx + 1, SortedEvictable),
          Bucket3 = lists:delete(EvictedValue, Bucket2),
          Buckets2 = array:set(BucketIdx, Bucket3, Buckets),
          Pool2 = Pool#pool{buckets = Buckets2},
          {free_space, [], EvictedValue, RandState2, Pool2}
      end
  end.
...
bucket_prepare([Value | Rest], FFun, KFun, BAcc, RAcc, EAcc, ECount) ->
  case FFun(Value) of
    keep ->
      bucket_prepare(Rest, FFun, KFun, [Value | BAcc], RAcc, EAcc, ECount);
    remove ->
      bucket_prepare(Rest, FFun, KFun, BAcc, [Value | RAcc], EAcc, ECount);
    evict ->
      BAcc2 = [Value | BAcc],
      EAcc2 = [{KFun(Value), Value} | EAcc],
...
-spec lookup_add(lookup(), term(), term())
  -> {non_neg_integer(), undefined | {non_neg_integer(), term()}, term(), lookup()}.
lookup_add(#lookup{size = 0} = Lookup, RandState, Value) ->
  {0, Lookup2} = lookup_append(Lookup, Value),
  {0, undefined, RandState, Lookup2};
lookup_add(#lookup{size = 1} = Lookup, RandState, Value) ->
  % even if there is a only two values we want the table randomized.
  {1, Lookup2} = lookup_append(Lookup, Value),
  {SwapFlag, RandState2} = randint(RandState, 2),
  case SwapFlag =:= 0 of
    false ->
      {1, undefined, RandState2, Lookup2};
    true ->
      {OldValue, _, Lookup3} = lookup_swap(Lookup2, 0, 1),
      {0, {1, OldValue}, RandState2, Lookup3}
  end;
lookup_add(Lookup, RandState, Value) ->
  {Idx, Lookup2} = lookup_append(Lookup, Value),
  {RandIdx, RandState2} = randint(RandState, Idx),
  {OldValue, _, Lookup3} = lookup_swap(Lookup2, RandIdx, Idx),
  {RandIdx, {Idx, OldValue}, RandState2, Lookup3}.
...
lookup_select(#lookup{size = 0}, RandState, _UseRandOffset, _FilterFun) ->
  {unavailable, RandState};
lookup_select(#lookup{size = 1} = Lookup, RandState, _UseRandOffset, undefined) ->
  {lookup_get(Lookup, 0), RandState};
lookup_select(Lookup, RandState, UseRandOffset, undefined) ->
  #lookup{size = Size} = Lookup,
  RandOffset = strong_randword(UseRandOffset),
  {RandInt, RandState2} = randint(RandState, Size),
  RandIdx = (RandInt + RandOffset) rem Size,
  {lookup_get(Lookup, RandIdx), RandState2};
lookup_select(Lookup, RandState, UseRandOffset, FilterFun) ->
  #lookup{size = Size} = Lookup,
  RandOffset = strong_randword(UseRandOffset),
  lookup_select(Lookup, RandState, FilterFun, RandOffset, Size).

lookup_select(Lookup, RandState, FilterFun, _Offset, 1) ->
  Value = lookup_get(Lookup, 0),
  case FilterFun(Value) of
    true -> {Value, RandState};
    false -> {unavailable, RandState}
  end;
lookup_select(Lookup, RandState, FilterFun, Offset, SamplingSize) ->
  LastIdx = SamplingSize - 1,
  {RandInt, RandState2} = randint(RandState, SamplingSize),
  RandIdx = (RandInt + Offset) rem SamplingSize,
  Value = lookup_get(Lookup, RandIdx),
  case FilterFun(Value) of
    true -> {Value, RandState2};
    false ->
      {_, _, Lookup2} = lookup_swap(Lookup, RandIdx, LastIdx),
      lookup_select(Lookup2, RandState2, FilterFun, Offset, SamplingSize - 1)
  end.
...
-spec lookup_sample(lookup(), rand_state(), boolean(), non_neg_integer() | all, int_filter_fun() | undefined)
  -> {[peer_id()], rand_state()}.
lookup_sample(#lookup{size = 0}, RandState, _UseRandOffset, _SampleSize, _FilterFun) ->
  {[], RandState};
lookup_sample(#lookup{size = Size} = Lookup, RandState, _UseRandOffset, SampleSize, undefined)
 when SampleSize =:= all; SampleSize >= Size ->
  #lookup{array = Array} = Lookup,
  {array:sparse_to_list(Array), RandState};
lookup_sample(Lookup, RandState, _UseRandOffset, all, FilterFun) ->
  #lookup{array = Array} = Lookup,
  ...
  {Result, RandState};
lookup_sample(Lookup, RandState, UseRandOffset, SampleSize, FilterFun) ->
  #lookup{size = Size} = Lookup,
  RandOffset = strong_randword(UseRandOffset),
  lookup_sample(Lookup, RandState, FilterFun, RandOffset, SampleSize, Size, []).

lookup_sample(_Lookup, RandState, _FilterFun, _Offset, 0, _SamplingSize, Acc) ->
  {Acc, RandState};
lookup_sample(Lookup, RandState, FilterFun, _Offset, Remaining, 1, Acc) ->
  Value = lookup_get(Lookup, 0),
  {_, Acc2} = lookup_filter(Value, FilterFun, Remaining, Acc),
  {Acc2, RandState};
lookup_sample(Lookup, RandState, FilterFun, Offset, Remaining, SamplingSize, Acc) ->
  {RandInt, RandState2} = randint(RandState, SamplingSize),
  ...
  lookup_sample(Lookup2, RandState2, FilterFun, Offset, Remaining2, LastIdx, Acc2).
