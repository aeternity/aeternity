...
%%% * local_pow - a PoW miner may be run locally by the node (but block
%%%   production/mining does not have to be started, in which case only
%%%   other nodes generate blocks)
...
%%% callback module such as `aec_consensus_bitcoin_ng`, which is encoded on
%%% the chain itself so it is known to all validators. For example, the
%%% beneficiary account should be constant for bitcoin-ng but for
%%% hyperchains it depends on the block height.
%%%
%%% The block production has two states of operation: 'running' or
%%% 'stopped'. Passing the option {autostart, bool()} to the initialization
%%% controls which mode to start in. In the stopped mode only blocks
%%% arriving from other miners are added to the chain. In the running mode,
%%% key block candidates are generated and then sealed by a separate
%%% worker.

%%% (In the context of PoW that would be producing a solution to the puzzle,
%%% and for PoS signing the block by the correct leader.) When it is
%%% successful, the new block is published and added to the chain if the
%%% state of the chain allows that.
%%%

%%% The microblock server in `aec_block_generator' collects transactions
%%% and builds a candidate microblock. When a microblock candidate is
%%% ready, this gets published as a `candidate_block` event, telling the
%%% conductor it can fetch the block.
%%%
...
hc_create_block(ConsensusModule, TopHash0, Leader) ->
    ...
    TopHash = hc_create_microblock(ConsensusModule, TopHash0, Leader, VoteResult),
