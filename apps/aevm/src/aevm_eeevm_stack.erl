...
push(Arg, State) ->
    ...
    if length(Stack) < 1024 ->
        ...
        true ->
            throw(?aevm_eval_stack_error(out_of_stack, State))
    end.
...
pop(State) ->
    case aevm_eeevm_state:stack(State) of
        ...
        [] ->
            throw(?aevm_eval_stack_error(error_pop_empty_stack, State))
    end.
...
dup(N, State) ->
    case aevm_eeevm_state:stack(State) of
        [] ->
            throw(?aevm_eval_stack_error(error_dup_empty_stack, State));
        Stack ->
            case length(Stack) < N of
                true ->
                    throw(?aevm_eval_stack_error(error_dup_too_small_stack, State));
                false ->
...
swap(N, State) ->
    case aevm_eeevm_state:stack(State) of
        [] ->
                throw(?aevm_eval_stack_error(error_swap_empty_stack, State));
        [Top|Rest] ->
            case length(Rest) < N of
                true ->
                    throw(?aevm_eval_stack_error(error_swap_too_small_stack, State));
                false ->
