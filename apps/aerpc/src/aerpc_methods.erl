...
dispatch_method(<<"web3_clientVersion">>, _Params) ->
    Vsn = aeapi:node_version(),
    Rev = aeapi:node_revision(),
    {ok, <<"aeternity/", Vsn/binary, "/", Rev/binary>>};

dispatch_method(<<"eth_blockNumber">>, _Params) ->
...
dispatch_method(<<"eth_accounts">>, _Params) ->
    %% Node never holds user wallet keys.
    {ok, []};

dispatch_method(<<"net_listening">>, _Params) ->
    %% If the node is able to answer this RPC it is, by definition, listening.
    {ok, true};

dispatch_method(<<"eth_getStorageAt">>, _Params) ->
...
dispatch_method(<<"eth_getUncleCountByBlockHash">>, _Params) ->
    %% AE has no uncles (Bitcoin-NG generations replace the uncle scheme).
    {ok, <<"0x0">>};

dispatch_method(<<"eth_getUncleCountByBlockNumber">>, _Params) ->
    %% AE has no uncles.
    {ok, <<"0x0">>};

dispatch_method(<<"eth_getUncleByBlockHashAndIndex">>, _Params) ->
    %% AE has no uncles.
    {ok, null};

dispatch_method(<<"eth_getUncleByBlockNumberAndIndex">>, _Params) ->
    %% AE has no uncles.
    {ok, null};

dispatch_method(<<"net_peerCount">>, _Params) ->
...
dispatch_method(<<"eth_protocolVersion">>, _Params) ->
...
dispatch_method(<<"eth_gasPrice">>, _Params) ->
...
dispatch_method(<<"eth_feeHistory">>, [BlockCountHex, NewestBlock])
...
dispatch_method(<<"eth_feeHistory">>,
...
dispatch_method(<<"eth_feeHistory">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_maxPriorityFeePerGas">>, _Params) ->
    aerpc_fee:max_priority_fee();

dispatch_method(<<"eth_chainId">>, _Params) ->
    NetworkId = aec_governance:get_network_id(),
    Numeric = aerpc_chain_id:to_numeric(NetworkId),
    {ok, aerpc_encoding:to_quantity(Numeric)};

dispatch_method(<<"net_version">>, _Params) ->
    %% Same numeric id as eth_chainId, but emitted as a decimal string
    %% per the net_version convention.
    NetworkId = aec_governance:get_network_id(),
    Numeric = aerpc_chain_id:to_numeric(NetworkId),
    {ok, integer_to_binary(Numeric)};

dispatch_method(<<"eth_getBlockByHash">>, [HashIn, FullTxs])
  when is_binary(HashIn), is_boolean(FullTxs) ->
    aerpc_block:by_hash(HashIn, FullTxs);
dispatch_method(<<"eth_getBlockByHash">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getBlockByNumber">>, [TagOrHex, FullTxs])
  when is_binary(TagOrHex), is_boolean(FullTxs) ->
    aerpc_block:by_height(TagOrHex, FullTxs);
dispatch_method(<<"eth_getBlockByNumber">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getBlockTransactionCountByHash">>, [HashIn])
  when is_binary(HashIn) ->
    aerpc_block:tx_count_by_hash(HashIn);
dispatch_method(<<"eth_getBlockTransactionCountByHash">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getBlockTransactionCountByNumber">>, [TagOrHex])
  when is_binary(TagOrHex) ->
    aerpc_block:tx_count_by_height(TagOrHex);
dispatch_method(<<"eth_getBlockTransactionCountByNumber">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getBalance">>, [AddrIn, BlockId])
  when is_binary(AddrIn), (is_binary(BlockId) orelse is_map(BlockId)) ->
    aerpc_account:balance(AddrIn, BlockId);
dispatch_method(<<"eth_getBalance">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getCode">>, [AddrIn, _TagOrHex])
...
dispatch_method(<<"eth_getCode">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getTransactionCount">>, [AddrIn, BlockId])
  when is_binary(AddrIn), (is_binary(BlockId) orelse is_map(BlockId)) ->
    aerpc_account:tx_count(AddrIn, BlockId);
dispatch_method(<<"eth_getTransactionCount">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getTransactionByHash">>, [HashIn])
  when is_binary(HashIn) ->
    aerpc_tx:by_hash(HashIn);
dispatch_method(<<"eth_getTransactionByHash">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getRawTransactionByHash">>, [HashIn])
  when is_binary(HashIn) ->
    aerpc_tx:raw_by_hash(HashIn);
dispatch_method(<<"eth_getRawTransactionByHash">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getTransactionByBlockHashAndIndex">>,
...
dispatch_method(<<"eth_getTransactionByBlockHashAndIndex">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getTransactionByBlockNumberAndIndex">>,
                [TagOrHex, IndexHex])
...
dispatch_method(<<"eth_getTransactionByBlockNumberAndIndex">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getTransactionReceipt">>, [HashIn])
  when is_binary(HashIn) ->
    aerpc_tx:receipt(HashIn);
dispatch_method(<<"eth_getTransactionReceipt">>, _Params) ->
    {error, -32602, <<"Invalid params">>};
...
dispatch_method(<<"eth_getBlockReceipts">>, [HashOrTag])
...
dispatch_method(<<"eth_getBlockReceipts">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_call">>, [TxObj, BlockId])
  when is_map(TxObj), (is_binary(BlockId) orelse is_map(BlockId)) ->
    aerpc_call:call(TxObj, BlockId);
dispatch_method(<<"eth_call">>, [TxObj]) when is_map(TxObj) ->
    aerpc_call:call(TxObj, <<"latest">>);
dispatch_method(<<"eth_call">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_estimateGas">>, [TxObj, BlockId])
  when is_map(TxObj), (is_binary(BlockId) orelse is_map(BlockId)) ->
    aerpc_call:estimate_gas(TxObj, BlockId);
dispatch_method(<<"eth_estimateGas">>, [TxObj]) when is_map(TxObj) ->
    aerpc_call:estimate_gas(TxObj, <<"latest">>);
dispatch_method(<<"eth_estimateGas">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_getLogs">>, [Filter]) when is_map(Filter) ->
...
dispatch_method(<<"eth_unsubscribe">>, _Params) ->
    {error, -32004,
     <<"Subscriptions require the WebSocket transport (/v3/rpc/ws)">>};
...
dispatch_method(<<"web3_sha3">>, _Params) ->
    {error, -32602, <<"Invalid params">>};

dispatch_method(<<"eth_sendRawTransaction">>, _Params) ->
...
dispatch_method(<<"eth_sendTransaction">>, _Params) ->
...
dispatch_method(<<"eth_sign">>, _Params) ->
...
dispatch_method(<<"eth_signTransaction">>, _Params) ->
    %% Signs a tx object with a node-held key and returns the RLP-encoded
    %% bytes. Out of scope for v1, same reasons as eth_sign +
    %% eth_sendTransaction: no node-held keys, no secp256k1 path, no
    %% Eth-RLP envelope. Real signing belongs in client-side wallets.
    {error, -32601, <<"Method not found">>};

dispatch_method(<<"eth_syncing">>, _Params) ->
