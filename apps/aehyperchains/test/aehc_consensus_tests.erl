...
%%% @doc
%%% Tests for deploying the hyperchains staking contract,
%%% using a predeployed staking contract, switching from PoW to HC
%%% @end
%%%-------------------------------------------------------------------
...
pos_block_structure_test_() ->
    Tests = protocol_gate([
        ...
        {"Key header with special nonce but no special key seal is a PoW block", fun key_header_special_nonce_no_seal_is_pow/0},
        {"Key header with special nonce and special key seal is a PoS block", fun key_header_special_nonce_special_seal_is_pos/0},
        {"Parent hash and miner signature setter/getter", fun parent_hash_and_signature_setter_getter/0}
    ]),
...
pos_keyheader_binary_roundtrip() ->
    ...
    try
        ...
        TxFuns =
            fun %% Deposit 1 AE at height 5
                (5) ->
                    [MkCallF(Patron, 1, 1 * ?AE, CallDepositStake)];
                (_) -> []
            end,
        ...
        meck:expect(aehc_utils, submit_commitment,
            fun(KeyNode, Delegate) ->
                ...
                CList = [C],
                CHList = [aehc_commitment:hash(Commitment) || Commitment <- CList],
...
two_delegates_at_10_criteria_1_1_5_0() ->
    try
        meck:new(aehc_utils, [passthrough]),
        aehc_consensus_hyperchains:set_hc_activation_criteria(1 * ?AE, 1, 5, 0),
        assert_static_staking_call_result({ok, false}, "enabled()"),
...
hc_switchover_at_10_test_() ->
    Tests = protocol_gate([
        {"Can mine some blocks. Accepts PoW blocks even with the global contract address available",
            fun can_mine_accepts_pow/0},
        {"When no predeployment was done then a system account will deploy the staking contract",
            fun no_predeployment_then_system_deploys_contract/0},
        {"Staking contract can be deployed by an ordinary user, until activated HC does not deviate from PoW",
            fun contract_deployed_by_user_hc_doesnt_deviate_from_pow/0}
    ]),
    aehc_test_utils:hc_chain_eunit_testcase(aehc_test_utils:pow_to_hc_switch(10), Tests).

can_mine_accepts_pow() ->
