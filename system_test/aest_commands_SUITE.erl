...
epoch_commands(Cfg) ->
    ...
    {0, Output1} = run_cmd_in_node_dir(node1, ["bin/epoch", "versions"], Cfg),
    ?assertMatch({match, _}, re:run(Output1,
        "Installed versions:[\r\n]*\\*[ \t]*[0-9\\.\\-a-z]*[ \t]*permanent[\r\n]*")),

    {0, Output2} = run_cmd_in_node_dir(node1, ["bin/epoch", "peer_key"], Cfg),
    ...
    {0, Output5} = run_cmd_in_node_dir(node1, ["bin/epoch", "ping"], Cfg),
    ?assertMatch({match, _}, re:run(Output5, "pong[\r\n]*")),

    Result6 = run_cmd_in_node_dir(node1, ["bin/epoch", "status"], Cfg),
    ?assertMatch({0, _}, Result6),

    {0, Output7} = run_cmd_in_node_dir(node1, ["bin/epoch", "keys_gen", "secret password"], Cfg),
    ...
    {0, _Output8} = run_cmd_in_node_dir(node1, ["bin/epoch", "export", GuestPath], Cfg),
