...
many_spend_txs(Cfg) ->
    setup_nodes([#{ name    => node1,
                    ...
                    config  => #{max_micro_blocks_sync => 4, persist => false}
                  },
                 #{ name    => node2,
                    peers   => [node1],
                    backend => aest_docker,
                    source  => {pull, "aeternity/epoch:local"},
                    config  => #{max_micro_blocks_sync => 1, persist => true}
                  },
                 #{ name    => node3,
                    peers   => [node1],
                    backend => aest_docker,
    ...
    #{node1 := [{_Hash, Height}]} = 
        wait_for_value({txs_on_chain, [LastTxHash]},
                   [node1], 10 * ?MINING_TIMEOUT, [{key_blocks, 1} | Cfg]),  %% wait one extra key blocks
    ...
    FoundNode3 = find_txs(node3, TxHashes, #{}, erlang:system_time(seconds) + 60),
    ...
    wait_for_startup([node2], Height + 1, Cfg),

    FoundNode2 = find_txs(node2, TxHashes, #{}, erlang:system_time(seconds) + 200),  %% around 10 micro blocks
    ?assertEqual(FoundNode2, FoundNode3),  %% may be different if we had a fork!
