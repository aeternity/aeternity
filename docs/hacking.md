      ...
      - `aec_conductor_sup`
        - Runs `aec_conductor` and `aec_block_generator`
          - The conductor is the hub for adding blocks to the local chain,
            both when received via gossip or sync, produced locally, or posted
            via the HTTP API. It is event driven, running tasks in background
            processes to ensure responsiveness.
          - The conductor has three modes: `local_pow` for Proof of Work
            (may run a local miner, but it does not have to start it);
            `pos` for Proof of Stake consensus, or `stratum`.
          - If local block production/mining is activated, the conductor
            will also drive the background mining (for PoW). If not, new blocks
            will only be generated by other nodes.
          - The block generator server receives requests to generate a new
            block and runs the production in the background. When a
            candidate block is ready, this fact gets published through
            aec_events, telling the conductor it can fetch the block.
          ...
          - Generating a key block candidate can take a little time, so it is
            also performed by a separate worker process. When it is done,
            this kicks off the block production. Under PoS, a candidate is
            only created when the local node decides it should produce a
            key block; for hyperchains, this depends on computed time slots,
            and the candidate worker may sleep until the right time. In this
            case the actual block production does not do any mining; it just
            signs the new key block. If candidate creation reports failure
            (e.g. due to not being leader), a fresh attempt will be made.
          - The details of the consensus strategy are parameterized into a
            separate module such as `aec_consensus_bitcoin_ng`, which is
            encoded on the chain itself so it is known to all validators.
            For example, the beneficiary account is constant for bitcoin-ng
            but for hyperchains it depends on the block height.
  - `aecli`
