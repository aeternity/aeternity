...
- All persistent state is kept in a Mnesia database, using RocksDB as the storage level backend. The `mnesia_rocksdb` app provides the Mnesia-to-RocksDB integration, and the `erlang-rocksdb` app provides the Erlang-to-C++ RocksDB bindings. On platforms where RocksDB is not supported, like on Windows NTFS volumes, a standard Mnesia database is used. The database is started early from a `$setup_hooks` entry in `aecore.app.src`, which calls `aec_db:start_db()`. (Read about setup hooks in [How the system starts](#How-the-system-starts) below.) The `aec_db:tables/1` function defines the database tables (the schema).
  - Apps should generally not call `mnesia` functions directly, but always go via `aec_db` or a higher level module like `aec_trees`.
