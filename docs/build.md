...
Then checkout the version to be build, e.g. `v0.11.0`:
...
Once the packaging is done, assuming the built version is `v0.11.0`, the package is created as `_build/prod/rel/epoch/epoch-0.11.0.tar.gz`.

To deploy the package for example in `/tmp/node` one should just unarchive it to that directory:

```bash
mkdir /tmp/node
tar xf _build/prod/rel/epoch/epoch-0.11.0.tar.gz -C /tmp/node
```
